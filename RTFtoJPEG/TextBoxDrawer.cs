...
namespace RTFtoJPEG
{
    ...
    {
        public static RichTextBoxDrawer rtfDrawer;
        ...
        public class RichTextBoxDrawer : RichTextBox
        {
            ...
            public void Draw(Graphics graphics, RectangleF layoutArea, float xFactor)
            {
                ...
                layoutArea.Width = layoutArea.Width * xFactor;
                layoutArea.Height = layoutArea.Height * xFactor + 10 * xFactor;

                for (int i = 0; i < rtfDrawer.Text.Length; i++)
                {
                    rtfDrawer.Select(i, 1);
                    rtfDrawer.SelectionFont = new System.Drawing.Font(rtfDrawer.SelectionFont.FontFamily, xFactor * rtfDrawer.SelectionFont.Size);
                }
                ...
                graphics.Clear(Color.White);
                graphics.FillRectangle(new SolidBrush(Color.White), layoutArea);
                graphics.CompositingMode = CompositingMode.SourceOver;
                ...
                metafile.Save(Program.fileName.Split('.')[0] + ".jpg", oCodecInfo, oParams);
            ...
            public void Draw(Graphics graphics, RectangleF layoutArea, float xFactor)
            {
                ...
                graphics.FillRectangle(new SolidBrush(Color.White), layoutArea);
                graphics.CompositingMode = CompositingMode.SourceOver;
                //graphics.DrawImage(metafile, layoutArea);//new Rectangle((int)layoutArea.Location.X, (int)layoutArea.Location.Y, (int)layoutArea.Size.Width, (int)layoutArea.Size.Height));
                using (var wrapMode = new ImageAttributes())
                {
                    wrapMode.SetWrapMode(WrapMode.TileFlipXY);
                ...
                MetafileHeader header = metafile.GetMetafileHeader();
                //graphics.ScaleTransform(header.DpiX / graphics.DpiX, header.DpiY / graphics.DpiY);
                graphics.PageUnit = GraphicsUnit.Pixel;
                //graphics.SetClip(new Rectangle(0, 0, (int)metaSize.Width, (int)metaSize.Height));
                EncoderParameters oParams = new EncoderParameters(1);
                oParams.Param[0] = new EncoderParameter(
                    System.Drawing.Imaging.Encoder.Quality, 100L);
                ImageCodecInfo oCodecInfo = GetEncoderInfo("image/jpeg");
                ...
                //Stream bmpStream;
                //Bitmap bitty;
                //using (bmpStream = File.Open(Program.fileName.Split('.')[0] + "_temp.bmp", System.IO.FileMode.Open))
                //{
                ...
                //
                //    bitty = new Bitmap(image.Width, image.Height);
                //    bitty.SetResolution(image.HorizontalResolution/xFactor, image.VerticalResolution/xFactor);
                //
                ...
                //    //bitty.Save(Program.fileName.Split('.')[0] + ".bmp", myImageCodecInfo, myEncoderParameters);
                //    using (var gra = Graphics.FromImage(bitty))
                //    {
                //        gra.Clear(Color.White);
                //        gra.DrawImageUnscaled(image, 0, 0);
                //        gra.FillRectangle(new SolidBrush(Color.White), new System.Drawing.Rectangle(0, 0, (int)(image.Width / xFactor), (int)(image.Height / xFactor)));
                ...
                //            //gra.Clear(Color.White);
                //            wrapMode.SetWrapMode(WrapMode.TileFlipXY);
                //            gra.DrawImage(image, new System.Drawing.Rectangle(0,0, image.Width, image.Height), 0, 0, image.Width, image.Height, GraphicsUnit.Pixel, wrapMode);
                //        }
                //        var codec = ImageCodecInfo.GetImageEncoders();
                //        
                //        //gra.DrawImage(image, image.Width/ xFactor, image.Height/ xFactor);
                //        bitty.Save(Program.fileName.Split('.')[0] + "_second_pass.jpg", myImageCodecInfo, myEncoderParameters);
                //    }
                //    bitty.Save(Program.fileName.Split('.')[0] + ".bmp", ImageFormat.Bmp);
                //    int width = (int)(image.Width / xFactor);
                //    int height = (int)(image.Height / xFactor);
                //    Bitmap bimmy = new Bitmap(width, height);
                //    Graphics graphy = Graphics.FromImage(bimmy);
                //    graphy.FillRectangle(new SolidBrush(Color.White), new Rectangle(0, 0, width, height));
                //    graphy.DrawImage(image, new Rectangle(0, 0, width, height));
                //    bitty.Save(Program.fileName.Split('.')[0] + ".bmp", ImageFormat.Jpeg);
                //    
                //    
                //}
                //File.Delete(Program.fileName.Split('.')[0] + "_temp.bmp");
            }*/
            ...
            private static class SafeNativeMethods
            {
                ...
                public const int WS_EX_TRANSPARENT = 0x00;//0x20;
